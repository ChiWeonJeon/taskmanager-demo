...
export function GroupTasksPageView({
  params,
  variant = "default",
}: {
  ...
  const tasksQueryKey = useMemo(() => ["work-items", "group", groupSlug], [groupSlug]);

  const groupQuery = useQuery<GroupDetail>({
    queryKey: ["project-group", groupSlug],
    queryFn: () => fetchJsonWithTimeout<GroupDetail>(`/api/project-groups/${groupSlug}`),
    retry: 1,
  });
  ...
  const tasksQuery = useQuery<WorkItemWithRelations[]>({
    ...
    queryFn: () =>
      fetchJsonWithTimeout<WorkItemWithRelations[]>(
        `/api/project-groups/${groupSlug}/work-items`,
      ),
    retry: 1,
  });
  ...
  const groupProjects = useMemo(() => groupQuery.data?.projects ?? [], [groupQuery.data?.projects]);
  const metadata = useMemo(
    ...
    [issueTypeSummaryQuery.data, issueTypesQuery.data, statusesQuery.data, tasks, groupProjects],
  );
  ...
  const isLoadFailed = tasksQuery.isError || groupQuery.isError;

  const groupProjectIds = useMemo(() => groupProjects.map((p) => p.id), [groupProjects]);
  ...
  const createMutation = useMutation({
    mutationFn: async ({
      ...
      if (!projectId) {
        throw new Error(messages.groupTasksPage.projectRequired);
      }
    ...
    onError: (err: Error) =>
      toast(err.message || messages.projectTasksPage.createError, { type: "error", sticky: true }),
  });
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async ({ itemId, data }) => {
      ...
      const previous = queryClient.getQueryData<WorkItemWithRelations[]>(tasksQueryKey);
      if (previous) {
        queryClient.setQueryData<WorkItemWithRelations[]>(tasksQueryKey, (old) =>
          (old ?? []).map((task) => {
            ...
            if (data.fieldValues) {
              ...
              for (const [fieldId, value] of Object.entries(data.fieldValues)) {
                ...
                if (idx >= 0) {
                  ...
                  existingFieldValues.push({
                    fieldId,
                    value: stringValue,
                    field: {} as WorkItemFieldValue["field"],
                  });
                }
              }
  ...
  return (
    <>
      ...
      <TaskWorkspace
        ...
        projects={projects}
        savedViewsWorkspaceKey={`tasks:group:${groupQuery.data?.group.id ?? groupSlug}${variant === "today" ? ":today" : ""}`}
        preferenceWorkspaceKey={`tasks:group:${groupSlug}${variant === "today" ? ":today" : ""}`}
        workspaceProjectId={undefined}
        defaultCreateProjectId={undefined}
        createProjectIds={groupProjectIds}
        ...
        onRefresh={() => queryClient.invalidateQueries({ queryKey: tasksQueryKey })}
      />
    </>
  );
}
