...
async function fetchWorkItems() {
  return fetchJsonWithTimeout<WorkItemWithRelations[]>("/api/work-items?assigneeId=me");
}
...
export function TasksPageView({ variant = "default" }: { variant?: "default" | "today" }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["work-items", "me"] });
      toast(messages.tasksPage.createSuccess, { type: "success" });
    },
    onError: (error: Error) => toast(error.message || messages.tasksPage.createError, { type: "error", sticky: true }),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_data, id) => {
      queryClient.invalidateQueries({ queryKey: ["work-items", "me"] });
      toast(messages.tasksPage.deleteSuccess, {
        type: "info",
        sticky: true,
        action: {
          label: messages.tasksPage.restore,
          onClick: async () => {
    ...
    onError: () => toast(messages.tasksPage.deleteError, { type: "error", sticky: true }),
  });
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async ({ id, data }) => {
      await queryClient.cancelQueries({ queryKey: ["work-items", "me"] });
      const previous = queryClient.getQueryData<WorkItemWithRelations[]>(["work-items", "me"]);
      if (previous) {
        queryClient.setQueryData<WorkItemWithRelations[]>(["work-items", "me"], (old) =>
          (old ?? []).map((task) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["work-items", "me"] });
      toast(messages.tasksPage.updateSuccess, { type: "success" });
    },
    onError: (err: Error, _variables, context) => {
      if (context?.previous) {
        queryClient.setQueryData(["work-items", "me"], context.previous);
      }
      toast(err.message || messages.tasksPage.updateError, { type: "error", sticky: true });
    },
  });
  ...
  return (
    <div className="min-w-0 w-full">
      ...
      <TaskWorkspace
        ...
        projects={projects}
        forceMyTasks
        savedViewsWorkspaceKey={variant === "today" ? "tasks:my:today" : "tasks:my"}
        preferenceWorkspaceKey={variant === "today" ? "tasks:my:today" : "tasks:my"}
        defaultCreateProjectId={personalProject?.id}
        onCreateTask={(payload) => createMutation.mutate(payload)}
        createPending={createMutation.isPending}
        onDelete={(id) => deleteMutation.mutate(id)}
        onUpdate={(id, data) => updateMutation.mutate({ id, data })}
        onRefresh={() => queryClient.invalidateQueries({ queryKey: ["work-items", "me"] })}
      />
    </div>
  );
}
